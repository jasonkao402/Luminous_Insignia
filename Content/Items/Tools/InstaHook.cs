...
namespace Luminous_Insignia.Content.Items.Tools
{
	internal class InstaHook : ModItem
	{
		public override void SetDefaults() {
			...
			Item.shootSpeed = 50f; // This defines how quickly the hook is shot.
			Item.shoot = ModContent.ProjectileType<InstaHookProjectile>(); // Makes the item shoot the hook's projectile when used.
		}
	...
	internal class InstaHookProjectile : ModProjectile
	{
		...
		*/
		...
		public override void NumGrappleHooks(Player player, ref int numHooks) {
			numHooks = hooksAmount; // The amount of hooks that can be shot out
		}

		// default is 11, Lunar is 24
		public override void GrappleRetreatSpeed(Player player, ref float speed) {
			speed = 80f; // How fast the grapple returns to you after meeting its max shoot distance
		}

		public override void GrapplePullSpeed(Player player, ref float speed) {
			speed = 150; // How fast you get pulled to the grappling hook projectile's landing position
		}
		...
		public override void GrappleTargetPoint(Player player, ref float grappleX, ref float grappleY) {
			// player.Center = new Vector2(grappleX, grappleY);
